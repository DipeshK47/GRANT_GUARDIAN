...
const main = async () => {
  ...
  const result = await autopilotService.authorizeFinalSubmit(args);
  logger.info(result, "Final submit authorization recorded");
};
...
main().catch((error) => {
  if (error instanceof Error) {
    logger.error(
      ...
      "Failed to record final submit authorization",
    );
  } else {
    logger.error({ error }, "Failed to record final submit authorization");
  }
  process.exit(1);
});
